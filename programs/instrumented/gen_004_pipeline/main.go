...
// description: Phase-21 instrumented variant of gen_004_pipeline (leak_bug=True).
// Stage3 exits after one recv; WeaveChan shows stage2 blocked on Send.
...
func main() {
	...
	go func() {
		defer wg3.Done()
		// Bug: exit early after 1 item, leaving stage2 blocked on Send
		val, _ := ch2.Recv()
		_ = val
	}()
