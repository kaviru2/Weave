...
Critically, this multi-turn training objective is distinct from standard
next-token autoregressive decoding at inference time: the training data
explicitly supervises cross-turn state coherence, which is why
single-step CE training fails to produce coherent multi-step rollouts
while trajectory-formatted training succeeds (§\ref{sec:ablation}).
...
While 130 source programs is modest compared to raw source-code
corpora, execution trace datasets are categorically more expensive
to produce: each program requires repeated nondeterministic runs
to capture scheduling variation, and each trace must be parsed,
segmented, and formatted into labelled prediction examples.
The 970 resulting training trajectories cover diverse concurrent
patterns (channel communication, mutex locking, select-blocking,
goroutine lifecycle) and are validated against 798 held-out predictions
from real production bugs — a quality constraint that raw code
corpora do not impose.
...
The Go runtime tracer records that a goroutine unblocked but does not record which channel receive or mutex unlock caused it. This creates an information-theoretic observability limit for any execution model: for certain synchronization patterns, the causal relationship is invisible, forcing $P(\eunblock) \approx 0$ structurally on plain traces. This is not a logging convenience or heuristic shortcut — the limit is structural, and no amount of additional training data or model capacity can overcome it on uninstrumented traces.
...
pipeline are publicly released~\cite{anon-artifacts}.
