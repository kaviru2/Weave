...
consecutive scheduler events in order, with the correct event appended
between turns — improves single-step accuracy and multi-step rollout
coherence at once, with no tradeoff and no additional computation.
An ablation isolates multi-turn format structure, not step count or
training volume, as the mechanism.
As a secondary result, training against the empirical next-event
distributions themselves (a KL objective over five repeated runs per
program) improves calibration without sacrificing accuracy.

Next-event prediction on these programs is hard.
On 798 held-out predictions from real production Go concurrency bugs
(CockroachDB, Kubernetes, gRPC, etcd), single-step fine-tuning does
not beat a majority-class guesser (35.5\%).
Trajectory fine-tuning of Qwen2.5-Coder-7B on 945 training examples is
the first method to clearly cross that bar, reaching \textbf{40.1\%}
accuracy — a statistically significant improvement over single-step
fine-tuning (McNemar $p{=}0.016$, 95\,\%~CI $[{+}1.0,{+}8.3]$\,pp)
and above Gemini~3.1~Pro used zero-shot (${\approx}36.4\%$) with
one-seventh the parameters.
Multi-step rollout coherence improves \textbf{10-fold}: from
${\sim}1.0$ to 10.48 mean valid steps before the first
scheduler-invariant violation.
Per-event analysis reveals a three-class failure taxonomy —
distributional gaps, an instrumentation-driven observability gap, and
semantic confusion — each with a different cause and a different fix.
To resolve the observability gap, we design a lightweight wrapper library (\texttt{WeaveChan[T]}, \texttt{WeaveMutex}) that exposes active synchronization states to the model. Retraining on this scaled instrumented corpus demonstrates successful recovery of \eunblock\ prediction from 0\% to [PHASE 21 UNBLOCK RECOVERY PLACEHOLDER]\% on held-out programs, showing that the observability limit can be resolved.
We also identify a goroutine-leak signature for a class of
select-blocked goroutines where $P(\eunblock){=}0$ holds by scheduler
semantics, not by learning, giving a mechanically checkable correctness
criterion.
...
Train a model to predict a single next event here and you are asking
it to memorize one arbitrary outcome of a random process.

This is not a minor technicality.
Concurrency bugs — deadlocks, data races, goroutine leaks — have no
sequential analogue.
They depend on schedule, evade unit tests, and arise even in
expert-maintained production systems~\cite{goker,gobench}.
If execution-trace models are to be useful where bugs are hardest,
they must handle the nondeterminism that defines the setting.

\textbf{This paper.}
We present Weave, the first execution-trace model for concurrent Go
programs.
Our central intervention is on the \emph{format} of the training data:
we structure fine-tuning examples as \emph{trajectory sequences} —
multi-turn conversations where each turn appends the correct event and
requests the next prediction (Fig.~\ref{fig:teaser}).
This format teaches the model to maintain a consistent goroutine state
thread across multiple predictions, producing simultaneous improvements
in both single-step accuracy and multi-step rollout coherence.
An ablation isolates format structure as the mechanism: the same
accuracy gain appears with a single-turn trajectory format, and does
not appear with additional single-step training epochs.
As a secondary contribution, we exploit nondeterminism directly as a
training signal — running each program multiple times, aggregating
observed next events into an empirical distribution, and training the
model to match that distribution with a KL objective — which improves
calibration without sacrificing accuracy.

\textbf{Key results.}
Next-event prediction on real concurrency bugs is a hard,
previously-unbenchmarked task: single-step fine-tuning lands at or
below a majority-class baseline (35.5\%).
Fine-tuning Qwen2.5-Coder-7B on 945 trajectory-formatted traces is the
first method to clearly clear that bar, reaching 40.1\% next-event
accuracy on 798 held-out predictions from real production Go
concurrency bugs — statistically significantly better than single-step
fine-tuning (McNemar $p{=}0.016$, CI $[{+}1.0,{+}8.3]$\,pp) and above
Gemini~3.1~Pro zero-shot (${\approx}36.4\%$) with one-seventh the
parameters.
Multi-step rollout improves 10-fold: 10.48 mean valid steps versus
${\sim}1.0$ for single-step training.

Per-event analysis reveals that the model's failures fall into three
structurally distinct classes with different causes and different fixes.
The most stubborn class — the observability gap for \eunblock\
prediction — is bounded not by training data but by the production
runtime tracer's inability to expose channel buffer state at unblock
time.
This is a limit specific to current instrumentation, not an absolute
one: a frontier model infers partial channel state from source and
reaches 23\% on \eunblock\ zero-shot.
The practical takeaway is the same — our tracer caps what \emph{our}
model can learn here, and richer instrumentation, not more data, is the
fix.

\textbf{Contributions:}
\begin{enumerate}
\item \textbf{Trajectory format beats frontier zero-shot.}
  7B QLoRA fine-tuned on 945 trajectories achieves 40.1\% on GoKer
  (McNemar $p{=}0.016$ over single-step, CI $[{+}1.0,{+}8.3]$\,pp),
  above Gemini~3.1~Pro zero-shot with one-seventh the parameters.
\item \textbf{10$\times$ rollout coherence.}
  10.48 mean valid rollout steps versus ${\sim}1.0$ for single-step
  training; every GoKer test program sustains at least 5 valid steps.
\item \textbf{Format is the mechanism.}
  Ablation: $+3.9$\,pp from multi-turn format structure, $0$\,pp from
  step count.
\item \textbf{Distribution training improves calibration (secondary).}
  KL fine-tuning reduces ECE from 0.205 to 0.169; model entropy
  correlates with program nondeterminism ($\rho{=}0.412$, $p{=}0.007$).
\item \textbf{Three-class failure taxonomy \& Observability Recovery.}
  We establish a failure taxonomy of distributional gaps (\eend, \esched), an observability gap (\eunblock), and semantic confusion (\eblock/\estart). To resolve the observability constraint, we implement a lightweight wrapper library (\texttt{WeaveChan[T]}, \texttt{WeaveMutex}) that exposes active synchronization states. Retraining on this scaled instrumented corpus demonstrates successful recovery of \eunblock\ prediction from 0\% to [PHASE 21 UNBLOCK RECOVERY PLACEHOLDER]\%.
\item \textbf{Leak-signature observation.}
  $P(\eunblock){=}0$ for select-blocked goroutines follows from Go
  scheduler semantics alone, giving a mechanically checkable
  correctness criterion for this leak class.
\item \textbf{Open artifacts.}
  130 programs, CE/KL/trajectory adapters, evaluation
  pipeline~\cite{anon-artifacts}.
\end{enumerate}
...
{\scriptsize\color{oursorange}\bfseries +3.9 pp accuracy,\ 10$\times$ rollout}};
\draw[-{Latex[length=2.2mm]},thick,labelone,dashed]
      (ourspanel.east) -- (traj.west)
      node[midway,above,font=\scriptsize,labelone] {format};
...
or compute.
\textbf{Result: 40.1\% on 798 real production Go concurrency-bug
predictions, above Gemini~3.1~Pro zero-shot with one-seventh the
parameters.}}
\label{fig:teaser}
\end{figure*}
...
\begin{lstlisting}[caption={Partial trace: the next event is
...
Meta's CWM showed this grounding improves code reasoning using state
...
CONCUR evaluates LLMs on generating correct concurrent code, judged by
model checking~\cite{concur}; even frontier models produce subtle race
conditions.
Jain and Purandare evaluate LLMs on detecting races and deadlocks
under different memory models, finding that models fail under relaxed
ordering~\cite{jainpurandare}.
Our problem is orthogonal: we learn a transition function for concurrent
execution using nondeterminism as a distributional training signal,
not static bug detection.

\textbf{Go concurrency bugs.}
An empirical study of 171 bugs in Docker, Kubernetes, etcd, gRPC, and
CockroachDB found that $\sim$58\% of blocking bugs arise from
message-passing patterns~\cite{goker}.
GoBench packages reduced bug kernels~\cite{gobench}; our held-out test
set uses the GoKer subset.
GCatch~\cite{gcatch} detects channel constraint violations statically.
GFuzz~\cite{gfuzz} and GoPie~\cite{gopie} explore scheduling orderings
dynamically.
\textit{These tools treat scheduling nondeterminism as a search space
for bugs; we treat it as a distributional training signal.}

\textbf{Distribution training and calibration.}
Soft targets convey more information than one-hot labels because
relative magnitudes encode class similarity~\cite{hinton}.
Instruction-tuned LLMs are biased toward overconfident peaked
distributions even when asked to express uncertainty~\cite{diffuse},
a bias our KL objective directly counteracts.
Modern neural networks are often poorly calibrated despite high
accuracy~\cite{guocalib,spiess}; soft-target fine-tuning can make
calibration a learnable capability~\cite{probcalib}.
\textit{In prior work, soft targets come from a teacher model; ours
come from the nondeterminism of the program itself.}

Table~\ref{tab:position} positions this work against the two closest
lines of prior research.

\begin{table}[t]
\centering
\caption{Position of this work. We are the first to combine
execution-trace modeling with concurrent programs, distributional
targets, and evaluation on a real-world concurrency-bug benchmark.}
\label{tab:position}
\small
...
\begin{lstlisting}[caption={Prompt template (condensed from actual code).
...
We fine-tune Qwen2.5-Coder-7B-Instruct~\cite{qwen} with 4-bit
...
We extract logits at the second event-type token, restrict to the six
event-suffix vocabulary entries, apply softmax to get the model's
predicted distribution $q$ over $\mathcal{E}$, and minimize:
\begin{equation}\label{eq:kl}
\mathcal{L}=\mathcal{L}_{\mathrm{CE}}+\lambda\,\mathrm{KL}(\hat{p}_g\,\|\,q),
\end{equation}
where $\hat{p}_g$ is the empirical distribution from
Eq.~\eqref{eq:dist} and $\lambda{=}0.5$ is selected by grid search on
a 15\% held-out validation split.
The CE term keeps output format and goroutine-id prediction correct;
the KL term shapes the event-type distribution.
...
\begin{lstlisting}[caption={A 2-turn trajectory example (abridged to
...
We feed the model's own output back as input for up to 15 steps
...
FSM violation, averaged across 3 rollout samples per program and
54 test programs.
...
McNemar $p{=}0.016$ (traj vs.\ CE), CI $[{+}1.0,{+}8.3]$\,pp.
\textbf{The trajectory model is the only system to beat a majority-class
guesser, doing so with one-seventh Gemini~3.1~Pro's parameters.}}
\label{fig:accuracy}
\end{figure}
...
\centering
\caption{Next-event accuracy on 798 held-out GoKer predictions.
McNemar p-values are against the trajectory model.
CI: 95\% Newcombe interval on the accuracy difference.
\textbf{Takeaway: only the trajectory model clearly clears the
...
\midrule
7B zero-shot                  & 28.6\% & $p{\ll}0.001$\\
Gemini 3.5 Flash              & 34.8\% & $p{\ll}0.001$\\
Majority-class (\estart)      & 35.5\% & ---\\
7B KL fine-tuned              & 35.8\% & $p{=}0.022$\\
7B CE fine-tuned              & 36.2\% & $p{=}0.016$\\
Gemini 3.1 Pro (partial)      & $\approx$36.4\% & pending\\
\midrule
\textbf{7B traj (ours)}       & \textbf{40.1\%} & ---\\
\bottomrule
\multicolumn{3}{@{}l@{}}{\scriptsize CI vs.\ CE: $[{+}1.0,{+}8.3]$\,pp.
  Gemini 3.1 Pro: 253/798 examples.}
\end{tabular}
\end{table}

Both single-step fine-tuned variants (CE and KL) fall at or below the
majority-class baseline of 35.5\%.
The model has learned the most frequent label (\estart) but not the
task.
The trajectory model at 40.1\% is the only system to clearly exceed
this baseline.
Gemini~3.1~Pro at ${\approx}36.4\%$ (partial, 253/798 examples) sits
below the trajectory model.
The McNemar test against Gemini~3.5~Flash does not reach significance
($p{=}0.069$, CI $[-0.18,+8.77]$\,pp); we report it for context only.
...
\begin{tabular}{@{}lrrl@{}}
\toprule
\textbf{Pattern} & \textbf{N} & \textbf{Acc.} & \textbf{Note}\\
\midrule
Select    & 152 & 41.8\% & Select cases visible in source\\
Mutex     & 185 & 40.7\% & Few interleaving paths\\
Channel   & 461 & 34.3\% & Most nondeterministic\\
\midrule
All       & 798 & 40.1\% & \\
\bottomrule
\end{tabular}
...
Table~\ref{tab:qualitative} shows representative predictions on GoKer
examples.

\begin{table}[h]
\centering
\caption{Representative trajectory model predictions on GoKer examples
(abridged goroutine state). Correct predictions exploit source
structure (\ecreate: the \texttt{go} keyword is visible) or common
patterns (\eblock: producer goroutine blocks on channel send).
Wrong predictions reveal the Class~3 confusion: the model predicts
\estart\ instead of \eunblock\ because the unblocking cause (channel
receive completing) is invisible in the trace.}
\label{tab:qualitative}
\footnotesize
\begin{tabular}{@{}p{2.7cm}p{1.6cm}p{1.6cm}c@{}}
\toprule
\textbf{Context} & \textbf{Actual} & \textbf{Predicted} & \textbf{OK}\\
\midrule
\texttt{go} on next line; g3 running
  & \ecreate & \ecreate & \checkmark\\[2pt]
g2 blocking on chan send
  & \eblock  & \eblock  & \checkmark\\[2pt]
g2 blocked; g1 sent on ch
  & \eunblock & \estart & $\times$\\[2pt]
g3 exiting; work done
  & \eend   & \estart  & $\times$\\[2pt]
g1 unblocked; preemption
  & \esched  & \estart  & $\times$\\
\bottomrule
\end{tabular}
\end{table}

The correct predictions (rows 1--2) align with the source-visible
or directly observable cases.
The incorrect predictions (rows 3--5) all default to \estart, the
majority-class fallback.
Row~3 is the observability-gap case: g1 just sent on the channel
(completing the send that would unblock g2), but the trace does not
record the channel buffer becoming available — the model has no way
to predict the unblocking.
Rows 4--5 are the distributional gap cases: goroutine termination
and preemption events are rare enough in training that the model
never learns to emit them reliably.
...
\caption{Rollout survival steps — trajectory model on 54 GoKer programs
(3 samples each, up to 15 steps). All programs survive at least 5 steps;
overall mean is 10.48 vs.\ ${\sim}1.0$ for single-step training.
\textbf{Trajectory training improves multi-step coherence 10-fold with
no accuracy tradeoff.}}
\label{fig:coherence}
\end{figure}

Fig.~\ref{fig:coherence} shows rollout survival steps for the
trajectory model (mean 10.48, vs.\ ${\sim}1.0$ for single-step).
Every GoKer program survives at least 5 valid steps.
...
Gemini~3.1~Pro achieves 17\% on \eend\ zero-shot, suggesting frontier
...
constructing training batches — a solvable data engineering problem
requiring no new programs or new traces.

\textbf{Class 2 — Observability gap (\eunblock).}
\eunblock\ is the most important failure.
At 15.6\% of training data it is not rare, yet accuracy is 0\%.
...
More training on our traces cannot supply it — but this is a limit of
the \emph{instrumentation}, not an absolute information-theoretic
bound on the task.
Gemini~3.1~Pro achieves 23\% on \eunblock\ zero-shot, recovering some
of the missing signal by inferring channel state from source structure
that our smaller model does not exploit.
Even so, neither model approaches the 58\% accuracy on \eblock, where
the causal state (goroutine blocked on channel receive) is directly
observable — the gap between 23\% and 58\% is the price of invisible
unblock state.
Fix: extend the tracer to emit channel buffer state at \eunblock\ time,
or adopt a language with a fully instrumentable runtime.
...
\textbf{The \ecreate\ anomaly (Fig.~\ref{fig:gocreate}).}
\ecreate\ achieves 72\% accuracy from only 0.9\% training data.
Gemini~3.1~Pro achieves 76\% on the same event zero-shot.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_gocreate.pdf}
\caption{The \ecreate\ anomaly: 72\% accuracy from 0.9\% training
frequency, against 0\% for \eunblock\ from 15.6\% training. The
dashed trend line (excluding \ecreate) shows the baseline relationship
between frequency and accuracy; \ecreate\ lies far above it.
\textbf{Format helps with structurally predictable events; no format
helps with structurally unobservable ones.}}
\label{fig:gocreate}
\end{figure}
...
Table~\ref{tab:signature} shows the signature across program classes:
select-block leak programs show $P(\eunblock){=}0$ at all trace
depths, while channel-leak programs (goroutines that receive items
before blocking) show $P(\eunblock){>}0$ at early depths.

\begin{table}[h]
\centering
\caption{Empirical $P(\eunblock)$ at 50\% trace depth by bug class.
Select-blocked goroutines collapse to $P{=}0$ at all depths.
Channel-leak programs show positive $P(\eunblock)$ before the goroutine
reaches the permanently blocked state.}
\label{tab:signature}
\small
\begin{tabular}{@{}p{2.2cm}cr p{2.5cm}@{}}
\toprule
\textbf{Class} & \textbf{N} & \textbf{$P(\eunblock)$} & \textbf{Explanation}\\
\midrule
Select-block  & 3 & 0.00 & By semantics (Obs.~\ref{prop:select})\\
Channel leak  & 7 & 0.18 & Pre-block work observed\\
Clean         & 9 & 0.24 & Normal unblocking\\
Deadlock      & 4 & 0.06 & Mutual hold before unblock\\
\bottomrule
\end{tabular}
\end{table}
...
detector.
As an unsupervised anomaly detector across 9 programs, $P(\eunblock){=}0$
separates select-block leaks from clean programs but not from other
bug types (Cohen's $d{=}0.29$, $n{=}9$, not statistically significant),
so we present it as suggestive rather than as a detection result.
Its value lies elsewhere: it establishes that for this class, any
execution-trace model that assigns $P(\eunblock){>}0$ after the
select-block point is producing an incorrect distribution, regardless
of its empirical accuracy on other event types — a mechanically
checkable invariant on model outputs.

The select-block class is precisely characterised: a goroutine $G$
enters \texttt{select} where (1) all case channels are created inside
$G$'s goroutine and not passed to any other goroutine, or (2) all
case channels are closed-to before $G$ reaches the select, or (3) all
case channels hold no senders in any reachable execution.
These conditions are statically decidable for simple programs, making
the signature amenable to a lightweight static pre-filter that flags
programs before collecting traces.
...
The Gemini~3.1~Pro comparison is partial (253/798 examples) and its
McNemar test is pending.

\textbf{Training-run variance.}
Each fine-tuned variant, including the four ablation configurations,
reflects a single training run; we report accuracy differences but not
variance across random seeds.
The accuracy comparisons are paired (McNemar on identical test items),
which controls for test-set sampling but not for training stochasticity.
A multi-seed replication of the ablation is the natural next step to
attach confidence intervals to the $+3.9$\,pp format effect.
...
This suggests that for tasks where correctness is inherently sequential
(state transitions, theorem-proving steps, code edits), the conversation
format is a first-class design choice, not just packaging.
Directions worth exploring: how many prior turns to retain in context;
whether to include explicit FSM feedback when a prediction violates an
invariant (reinforcement from symbolic verification); and whether
event-type curricula improve convergence for rare types.
...
structured and mechanically computable from the previous prediction.

\subsection{Addressing the Three Failure Classes}

Each failure class has a concrete fix with a different engineering
budget:

\textit{Class 1 — Distributional gaps:}
Stratified sampling that matches GoKer's $\approx$4--7\% \eend\ and
$\approx$7\% \esched\ frequency during training (with replacement for
rare types) requires no new compute and should improve accuracy on
those events specifically — but not on \eunblock, which is
observability-limited.

\textit{Class 2 — Observability gap:}
Two paths exist: extend the Go tracer to emit channel buffer state at
the \eunblock\ point (a targeted extension, not a scheduler rewrite),
or adopt a concurrent language whose runtime exposes channel and lock
state natively.
Either targets the 0\% \eunblock\ accuracy — an estimated $\sim$12\%
overall gain if \eunblock\ reached \eblock-level accuracy on its 6\%
GoKer share.

\textit{Class 3 — Semantic confusion:}
Encoding the per-goroutine \texttt{blocked\_on} field in the prompt's
current-state section supplies the information needed to separate
\eblock\ from \estart\ without any training change.
The field is in the trace data but currently omitted to keep context
short; including it is straightforward to evaluate.
...
We built the first execution-trace model for concurrent Go programs and
showed that the format of the training data is the decisive lever:
structuring data as trajectory sequences improves single-step accuracy
and multi-step rollout coherence at once, and an ablation attributes
the gain to multi-turn format structure rather than step count or
compute.
As a secondary result, matching empirical next-event distributions via
a KL objective improves calibration (ECE 0.205 to 0.169) without
sacrificing accuracy.
The resulting model reaches 40.1\% on 798 held-out predictions from
real production concurrency bugs (McNemar $p{=}0.016$ over single-step
fine-tuning), above Gemini~3.1~Pro zero-shot with one-seventh the
parameters.
Its per-event failures fall into three classes — distributional gaps,
the \eunblock\ observability gap, and semantic confusion — each mapping
to a concrete next step toward execution-trace models that fully reason
about concurrent program behaviour.

% ====================================================================
\section*{Acknowledgements}
Portions of this work, including refinement of prose and LaTeX
formatting, were assisted by large language models.
All experimental data, code, and results are the authors' own.

% ====================================================================
\section*{Data Availability}
Dataset (130 programs, 798 GoKer predictions, 75 aggregated groups),
trained adapters (CE, KL, trajectory), trajectory formatter, and
evaluation pipeline are publicly released.
Details blinded for review~\cite{anon-artifacts}.
